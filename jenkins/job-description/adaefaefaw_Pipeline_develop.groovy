...
def job_config = [
        job: [
                name: "adaefaefaw_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'adaefaefaw_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/adaefaefaw_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.adaefaefaw.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "adaefaefaw", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("adaefaefaw_Pipeline_develop");

job.with {

    authenticationToken('ZKHgcxlFi2DBHPHiAk8VGAtNmvzBzoLq')
